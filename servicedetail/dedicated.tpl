...
/* Server Header Box */
.server_header_box {
  background-image: linear-gradient(87deg, var(--primary) 0%, var(--primary-dark) 100%);
  border-radius: 15px;
  padding: 25px;
  color: var(--light);
...
/* Chart Select */
select#chartSelect0, select#chartSelect1, select#chartSelect2 {
  border-radius: 50px;
...
select#chartSelect0:focus, select#chartSelect1:focus, select#chartSelect2:focus {
  border-color: var(--primary);
...
.modal-footer {
  border-top: 1px solid rgba(240, 138, 93, 0.2);
  padding: 20px 25px;
}
          ...
          <div class="row">
            ...
            <div class="col-12 mb-3">
              <div class="bg-light card-body bg-gray">
                ...
                <p class="mb-2">{$Lang.user_name}：{$Detail.host_data.username}</p>
                <p class="mb-2">{$Lang.password}：
            ...
            <div class="col-12 mb-3">
              <div class="bg-light card-body bg-gray">
                ...
                <p class="mb-2">{$Lang.panel_manage_address}：{$temp_custom_field_data['panel_address'] ??
          ...
          <div class="row">
            <div class="col-12 mb-3">
              <div class="bg-light card-body bg-gray pre-money-box">
                ...
                <section class="d-flex align-items-center flex-wrap mb-3">
                ...
                <section class="text-gray">
                  <p class="mb-2">{$Lang.subscription_date}：{$Detail.host_data.regdate|date="Y-m-d H:i"}</p>
                  <p class="mb-2">{$Lang.payment_cycle}：{$Detail.host_data.billingcycle_desc}</p>
                  {if $Detail.host_data.billingcycle == 'free' || $Detail.host_data.billingcycle == 'onetime'}
                  <p class="mb-0">{$Lang.due_date}：-</p>
                  {else}
                  <p class="mb-0">{$Lang.due_date}：{$Detail.host_data.nextduedate|date="Y-m-d H:i"}</p>
                  {/if}
                </section>
              </div>
            </div>
        ...
        <div class="card-body">
          ...
          <div class="tab-content p-3 text-muted">
            ...
            $Detail.host_data.allow_upgrade_product)) && !$Detail.download_data && !$Detail.host_data.show_traffic_usage}active{/if}"
              ...
              <!-- Journal content will be loaded dynamically -->
            </div>
            ...
            $Detail.host_data.allow_upgrade_product)) && !$Detail.download_data}active{/if}" id="dosage"
              ...
              <div class="row d-flex align-items-center mb-4">
                ...
                <span class="mx-2">{$Lang.reach}</span>
                <div class="col-md-3">
            ...
            <div class="tab-pane" id="finance" role="tabpanel">
              <!-- Finance content will be loaded dynamically -->
            </div>
          </div>
        </div>
      </div>
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        for (let i = 0; i < chartsData.length; i++) {
          ...
          str += `<div class="w-100 h-100 mb-4">
                    ...
                    <div style="height: 500px" class="w-100 h-100 d-flex justify-content-center chart_content_box" id="balanceCharts${i}"></div>
                  </div>`
        }
        $('#home1').append(str);
        ...
        },
        {
          ...
          areaStyle: {
            normal: {
              color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{
                offset: 0,
                color: 'rgba(255, 190, 118, 0.3)'
              }, {
                offset: 1,
                color: 'rgba(255, 190, 118, 0.1)'
              }]),
              opacity: 0.8
            }
          },
...
<script>
  ...
  $(document).on('click', '#logininfo', function () {
    ...
    $('#popContent').html(`
      <div class="mb-2">{$Lang.user_name}：{$Detail.host_data.username}</div>
      <div class="mb-2">
