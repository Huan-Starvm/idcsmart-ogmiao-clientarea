<style>
  ...
  .downloadTemplate:hover {
    ...
    color: var(--primary);
    border-left: 1px solid rgba(240, 138, 93, 0.2);
	}
  
  /* Certification type boxes */
  .verified_not_certified {
    padding: 20px 0;
  }
  ...
  #qrcode:hover, #qrcodeEnterprises:hover {
	...
	<div class="card">
		...
		{if $Verified.invoiceid} <!-- 未支付 -->
			{if $Verified.freetimes>0}
              ...
              <b>{$Lang.this_certification_free}</b> <span class="text-muted ml-2">{$Lang.remaining_free_times}: {$Verified.freetimes_use}/{$Verified.freetimes}</span>
            </div>
					</div>
					<div class="card-body text-center border-top">
						...
						{if $QrcodeUrl} <!-- 支付宝做特殊处理 -->
							<h5 class="pt-2 font-weight-bold h5 py-4">{$Lang.please_scan_alipay_qrcode}</h5>
				...
				{else}
					<div class="card-body text-center d-flex justify-content-center py-5 border-top">
						<div>
							<b>{$Lang.this_certification_requires_payment}:</b>
							<span class="h2 font-weight-bold text-primary">{$Verified.total}{$Lang.element}</span>
							<span class="ml-3">({$Lang.free_times}: {$Verified.freetimes_use}/{$Verified.freetimes}）</span>
							<a href="javascript: payamount({$Verified.invoiceid},0);" class="btn btn-primary py-1 px-4 ml-3" role="button">{$Lang.go_to_pay}</a>
						</div>
					</div>
				{/if}
			{else}
		...
		{else}
            ...
            <span class="h4 font-weight-bold m-0">{$Lang.payment_successful}</span>
          </div>
				</div>
...
<div class="card ststus-box">
	<div class="card-body p-5">
		<div class="container-fluid mt-0">
          ...
          <h5 class="pt-4 font-weight-bold" style="margin-bottom: 20px;">{$Lang.member_information}</h5>
